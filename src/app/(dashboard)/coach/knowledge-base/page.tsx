...
export default async function CoachKnowledgeBasePage() {
  ...
  const featureEnabled = await isFeatureEnabled('KNOWLEDGE_BASE', 'COACH', session.user.id)
  if (!featureEnabled) {
    return (
  ...
  const [categoriesResult, articlesResult] = await Promise.all([
    getKBCategories('COACH'),
    getKBArticles({ role: 'COACH', status: 'PUBLISHED', limit: 10 }),
  ])
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Browse articles and documentation to help you succeed
        </p>
      </div>
              ...
              {categories.length === 0 ? (
                ...
                <div className="space-y-2">
                  {categories.map((category) => (
                    <Link
                      key={category.id}
                      href={`/coach/knowledge-base/category/${category.slug}`}
              ...
              {recentArticles.length === 0 ? (
                ...
                <div className="space-y-4">
                  {recentArticles.map((article) => (
                    <Link
                      key={article.id}
                      href={`/coach/knowledge-base/${article.slug}`}
